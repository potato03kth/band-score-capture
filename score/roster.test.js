...
test('buildFinalMapping: 학번이 채워진 경우 matched, 안 채워진 경우 synthetic', () => {
  ...
  // 순번은 전체 후보(userNo 1,2) 기준이라 2는 TEST0002(전체 오름차순 2번째) — userNo 1이
  // 먼저 채워졌다고 해서 남은 후보의 순번이 앞당겨지지 않아야 재실행 간 안정성이 보장된다.
  assert.equal(result.get(2).studentId, 'TEST0002');
});
...
test('buildFinalMapping: 재실행 사이 일부 후보가 채워져도 남은 후보의 합성 학번은 안정적으로 유지된다(NU5)', () => {
  const candidates = [
    { userNo: 1001, name: '김철수', bandId: '111', bandName: '1반' },
    { userNo: 1002, name: '이영희', bandId: '111', bandName: '1반' },
    { userNo: 1003, name: '박민수', bandId: '111', bandName: '1반' },
    { userNo: 1004, name: '최영수', bandId: '111', bandName: '1반' },
  ];
  const run1 = roster.buildFinalMapping(candidates, new Map());
  assert.equal(run1.get(1002).studentId, 'TEST0002');
  assert.equal(run1.get(1004).studentId, 'TEST0004');

  // 2차 실행: 1001·1003만 학번이 채워짐(교수가 일부만 먼저 채운 상황)
  const run2 = roster.buildFinalMapping(candidates, new Map([[1001, '20260001'], [1003, '20260003']]));
  assert.equal(run2.get(1002).studentId, 'TEST0002');
  assert.equal(run2.get(1004).studentId, 'TEST0004');
});
