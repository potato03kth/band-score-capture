...
test('buildRows: assignmentScores를 안 넘기면 과제제출수/과제점수는 0이다(기존 호출부 회귀 안전)', () => {
  const mapping = new Map([
    [1, { userNo: 1, name: 'A', bandId: '111', bandName: '1반', studentId: 's1', mappingStatus: 'matched' }],
  ]);
  const rows = csv.buildRows({ mapping, scores: new Map(), cap: 50, bandCollectionComplete: new Map([['111', true]]), generatedAt: 'now' });
  assert.equal(rows[0].과제제출수, 0);
  assert.equal(rows[0].과제점수, 0);
});

test('buildRows: assignmentScores의 assignmentPostCount/assignmentScore가 과제제출수/과제점수로 옮겨지고, 점수(활동점수)와는 합산되지 않는다', () => {
  const mapping = new Map([
    [1, { userNo: 1, name: 'A', bandId: '111', bandName: '1반', studentId: 's1', mappingStatus: 'matched' }],
  ]);
  const scores = new Map([[1, { activeDays: 2, postCount: 1, commentCount: 1, score: 2, records: [] }]]);
  const assignmentScores = new Map([[1, { assignmentPostCount: 3, assignmentScore: 15, records: [] }]]);
  const rows = csv.buildRows({ mapping, scores, assignmentScores, cap: 50, bandCollectionComplete: new Map([['111', true]]), generatedAt: 'now' });
  assert.equal(rows[0].과제제출수, 3);
  assert.equal(rows[0].과제점수, 15);
  assert.equal(rows[0].점수, 2); // 활동점수는 과제점수와 무관하게 그대로
});
...
test('writeAuditWorkbook: _assignmentRecords는 감사 시트에 유형 "과제제출"로 별도로 나열된다', async () => {
  const dir = makeTmpDir();
  try {
    const filePath = path.join(dir, 'audit.xlsx');
    const rows = [
      {
        ...
        감사근거: '1건 활동',
        산출시각: 'now',
        _records: [{ dateStr: '2026-06-28', kind: 'post', postNo: '1', textPreview: '글1' }],
        _assignmentRecords: [{ dateStr: '2026-06-29', postNo: '5', textPreview: '과제1 제출합니다' }],
      },
    ];
    ...
    await wb.xlsx.readFile(filePath);
    const auditSheet = wb.getWorksheet('감사');
    assert.equal(auditSheet.rowCount, 3); // 헤더 1 + 활동 1건 + 과제제출 1건
    assert.equal(auditSheet.getRow(2).getCell(5).value, '게시글');
    assert.equal(auditSheet.getRow(3).getCell(5).value, '과제제출');
    assert.equal(auditSheet.getRow(3).getCell(6).value, '5');
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});
...
test('writeAuditWorkbook: 결과 시트에서 활동일수/게시글수/댓글수/점수가 0인 셀만 배경(fill)이 설정된다', async () => {
  ...
  try {
    ...
    const zeroCols = ['활동일수', '게시글수', '댓글수', '점수', '과제제출수', '과제점수'].map(colIndex);
