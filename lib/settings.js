...
const BAND_SHEET_HEADERS = ['밴드 이름', '밴드 URL 또는 ID', '예상 게시글수(선택)'];
...
// CLI/env에서 실행모드를 읽는다. 엑셀·개발자 config 어느 쪽에도 두지 않는다 —
// 교수는 acquire/main.js·score/index.js를 직접 실행하지 않고, 매 실행마다 사람이 직접
// 넣어주는 값이라 CLI/env가 맞다(H-1, 2026-07-21). --mode=test가 BSC_MODE보다 우선한다.
// 아무 것도 주지 않으면 production(스테일 설정으로 실행하는 실수를 막기 위한 안전한 기본값).
function resolveMode({ argv = process.argv, env = process.env } = {}) {
  ...
  if (raw == null && env.BSC_MODE) raw = env.BSC_MODE;
  if (raw == null) return 'production';

  const mode = String(raw).trim().toLowerCase();
  if (!VALID_MODES.includes(mode)) {
    throw new SettingsValidationError(
      `실행 모드 값이 올바르지 않습니다. test 또는 production만 허용됩니다(--mode=test 또는 BSC_MODE=test). 현재 값: ${JSON.stringify(raw)}`
    );
  }
  return mode;
}

async function loadSettings({ inputDir, mode }) {
  if (!VALID_MODES.includes(mode)) {
    throw new SettingsValidationError(
      `loadSettings에 전달된 mode 값이 올바르지 않습니다(내부 호출 오류). 현재 값: ${JSON.stringify(mode)}`
    );
  }
  ...
  bandSheet.eachRow({ includeEmpty: false }, (row, rowNumber) => {
    ...
    const expectedRaw = xlsx.readCell(row.getCell(3));
    ...
    let expectedPostCount = null;
    if (expectedRaw !== '' && expectedRaw != null) {
      const n = Number(expectedRaw);
      if (!Number.isFinite(n) || n <= 0 || !Number.isInteger(n)) {
        throw new SettingsValidationError(
          `"대상밴드" 시트 ${rowNumber}행의 "예상 게시글수" 값이 올바르지 않습니다. 비워두거나 1 이상의 정수를 입력하세요. 현재 값: ${JSON.stringify(
            expectedRaw
          )}`
        );
      }
    ...
    bands.push({ name: String(name).trim(), bandId, expectedPostCount });
  });
