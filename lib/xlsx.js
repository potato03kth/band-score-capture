...
// input/score_logic.xlsx — Phase 8(최하위 우선순위, 확장성 목적). 채점 공식의 5개 변수를
// 노출한다: 일일활동상한(하루에 셀 활동 건수 자체를 자르는 상한, 배수 적용 전)·일일점수상한
// (배수 적용 후 그날 점수의 상한 — 기존 하드코딩된 "하루 최대 1점"의 자리)·댓글배수·게시글배수
// (기존엔 게시글/댓글 동일배점=1)·과제글포함여부(기존엔 규칙4로 항상 제외). 값은 전부
// "변경불필요" 기본값 — 손대지 않으면 lib/scoreLogic.js가 이걸 옛 하드코딩 값 그대로로
// 해석해 기존 채점 결과와 100% 동일하게 나온다(H-3 Phase 8 요구사항).
async function createScoreLogicTemplate(filePath) {
  await ensureDir(path.dirname(filePath));
  const wb = new ExcelJS.Workbook();

  const sheet = wb.addWorksheet('설정');
  sheet.columns = [
    { header: '항목', key: 'item', width: 16 },
  ...
  const rows = [
    ['일일활동상한', '변경불필요', '하루에 점수로 셀 활동 건수의 상한(배수 적용 전). 예: 3이면 하루 4건째부터는 무시. 숫자 또는 "변경불필요"(무제한)'],
    ['일일점수상한', '변경불필요', '배수 적용 후, 하루가 줄 수 있는 점수의 상한. "변경불필요"=1점(기존 규칙과 동일)'],
    ['댓글배수', '변경불필요', '댓글(대댓글 포함) 1건이 몇 점인지. "변경불필요"=1(게시글과 동일배점)'],
    ['게시글배수', '변경불필요', '게시글 1건이 몇 점인지. "변경불필요"=1(댓글과 동일배점)'],
    ['과제글포함여부', '변경불필요', '교수 과제글([과제n])에 달린 댓글도 채점에 포함할지. "포함" / "제외" / "변경불필요"(=제외, 기존 규칙4와 동일)'],
    ['과제글당점수', '변경불필요', '학생이 과제글(교수 [과제n] 게시글)에 댓글을 단 과제 1개당 점수(같은 과제글에 여러 번 달아도 1개로 침). "변경불필요"=5점'],
    ['과제점수상한', '변경불필요', '과제점수(활동점수와 별도 합산되는 컬럼)의 상한. "변경불필요"=20점'],
  ];
  for (const [item, value, desc] of rows) {
    const row = sheet.addRow({ item, value, desc });
    applyYellow(row.getCell('value'));
  }
