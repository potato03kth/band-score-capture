...
test('createScoreLogicTemplate: 5개 항목이 전부 "변경불필요" 기본값·노란칸으로 채워진다', async () => {
  const dir = makeTmpDir();
  try {
    const filePath = path.join(dir, 'score_logic.xlsx');
    await xlsx.createScoreLogicTemplate(filePath);
    const wb = await xlsx.readWorkbook(filePath);
    const sheet = wb.getWorksheet('설정');
    assert.equal(xlsx.readCell(sheet.getRow(1).getCell(1)), '항목');

    const items = [];
    sheet.eachRow({ includeEmpty: false }, (row, rn) => {
      if (rn === 1) return;
      items.push(xlsx.readCell(row.getCell(1)));
      assert.equal(xlsx.readCell(row.getCell(2)), '변경불필요');
      assert.equal(row.getCell(2).fill.fgColor.argb, 'FFFFFF00'); // 노란칸
    });
    assert.deepEqual(items, ['일일활동상한', '일일점수상한', '댓글배수', '게시글배수', '과제글포함여부']);
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});
