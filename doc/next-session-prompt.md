...
- 멤버별 대조의 남은 불일치(4분반 48/56명, diff 대부분 1)는 §28-11에서 원인 규명: 측정 기간
  (06-28~07-16) 밖에 있는 고참여 글(특히 06-22의 "자기소개"성 글, comment_count=49)에
  대부분의 멤버가 댓글을 달았는데, `verify_member_comment_counts.js`가 비교하는 멤버의
  "댓글" 탭 표시값은 전체 기간 기준이라 생기는 것 - **버그 아님, 오히려 측정 기간 설정이
  의도대로 정확히 동작하는 증거**. `verify_member_comment_counts.js` 자체가 기간 미스매치인
  비교를 하고 있다는 게 진짜 남은 이슈(다음에 고칠 후보, 급하지 않음).
